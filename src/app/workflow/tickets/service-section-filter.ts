...
import { serviceSectionInfoMap, ServiceSectionName } from './ticket-resource';
import { Badge } from '../../shared/components/ui/badge';

@Component({
  selector: 'app-service-section-filter',
  imports: [Dropdown, Badge],
  template: `
    <app-dropdown class="inline-block relative" className="absolute right-0 z-10">
      <div dropdown-button>
      ...
      <div dropdown-content>
        @for (serviceSection of selectedSections(); track $index) {
          <app-badge
            size="sm"
            [endIcon]="closeIcon"
            [color]="SERVICE_SECTION_INFO_MAP[serviceSection.name].color"
            (iconClick)="removeSection($event, serviceSection.name)"
          >
            {{ SERVICE_SECTION_INFO_MAP[serviceSection.name].label }} ( {{ serviceSection.count }} )
          </app-badge>
        }
...
export class ServiceSectionFilter {
  readonly selectedSections = model.required<{ name: ServiceSectionName; count: number }[]>();
  ...
  removeSection(event: Event, section: ServiceSectionName) {
    event.stopPropagation();
    this.selectedSections.update((sections) => sections.filter((s) => s.name !== section));
  }
}
