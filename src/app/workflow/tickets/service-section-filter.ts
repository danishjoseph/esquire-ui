      ...
      <div dropdown-content>
        @for (serviceSection of selectedSections(); track $index) {
          <app-badge
            size="sm"
            [endIcon]="closeIcon"
            [color]="serviceSectionInfoMap[serviceSection].color"
            (iconClick)="removeSection($event, serviceSection)"
          >
            {{ serviceSectionInfoMap[serviceSection].label }}
          </app-badge>
        }
...
export class ServiceSectionFilter {
  readonly selectedSections = model.required<ServiceSectionName[]>();
  ...
  removeSection(event: Event, section: ServiceSectionName) {
    ...
    this.selectedSections.update((sections) => sections.filter((s) => s !== section));
  }
