      ...
      <div dropdown-content>
        @for (serviceSection of selectedSections(); track $index) {
          <app-badge
            size="sm"
            [endIcon]="closeIcon"
            [color]="serviceSectionInfoMap[serviceSection.sectionName].color"
            (iconClick)="removeSection($event, serviceSection.sectionName)"
          >
            {{ serviceSectionInfoMap[serviceSection.sectionName].label }} (
            {{ serviceSection.count }} )
          </app-badge>
        }
...
export class ServiceSectionFilter {
  readonly selectedSections =
    model.required<{ sectionName: ServiceSectionName; count: number }[]>();
  ...
  removeSection(event: Event, section: ServiceSectionName) {
    ...
    this.selectedSections.update((sections) => sections.filter((s) => s.sectionName !== section));
  }
